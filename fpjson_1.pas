...
Type
  ...
  public
    ...
    function AsjsonIdent(ident: string=''): string;virtual;
  ...
  public
    function AsjsonIdent(ident:String=''):string;override;
...
function TJSONData.AsjsonIdent(ident: string=''): string;
begin
  result:= self.AsJSON;
end;
...
function TJSONArray.AsjsonIdent(ident: string): string;
Var
  I  : Integer;
  Sep , ident2: String;
begin
  ...
  For I:=0 to Count-1 do
    begin
    Result:=Result+#13+ident2+Items[i].AsjsonIdent(ident2);
    If (I<Count-1) then
      Result:=Result+Sep;
    end;
  Result:=Result+#13+ident+']';
end;
...
function TJSONObject.AsjsonIdent(ident: string): string;
Var
  I  : Integer;
  Sep , ident2{, str1,str2}: String;
begin
  Sep:=TJSONData.FElementSep;
  Result:='';
  ident2:=ident+'    ';
  ...
  begin
    If (Result<>'') then
      Result:=Result+Sep;
    //if result ='' then
    //  Result:='  '+FElementStart+StringToJSONString(Names[i])+FElementEnd+Items[I].AsjsonIdent(ident2)
    //else
      Result:=Result+#13+ident2+FElementStart+StringToJSONString(Names[i])+FElementEnd+Items[I].AsjsonIdent(ident2);
  end;
  //str1:=FObjStartSep; str2:=FObjEndSep;
  If (Result<>'') then
    Result:=#13+ident+FObjStartSep+Result+#13+ident+trim(FObjEndSep)
  else
    Result:=#13+ident+'{}';
